...
#SBATCH -t 0-8:00:00
#SBATCH -J crysallm
#SBATCH --mem=640000 # Memory pool for all cores in MB
##SBATCH --gres=gpu:h100:4
#SBATCH -e train_N2n4_ultra_%j.err
#SBATCH -o train_N2n4_ultra_%j.out # File to which STDOUT will be written %j is the job #
...
# srun torchrun --nproc_per_node=2 --nnodes=2 --rdzv_id=$MASTER_PORT --rdzv_endpoint=$MASTER_ADDR --rdzv_backend=c10d bin/train_ddp.py --config=config/crystallm_v2_large.yaml
# srun torchrun --nproc_per_node=4 --nnodes=2 --rdzv_id=$RANDOM --rdzv_endpoint=$head_node_ip:29500 --rdzv_backend=c10d bin/train_ddp.py --config=config/crystallm_v3_large_N2n4.yaml
srun torchrun --nproc_per_node=4 --nnodes=2 --rdzv_id=$RANDOM --rdzv_endpoint=$head_node_ip:29500 --rdzv_backend=c10d bin/train_ddp.py --config=config/crystallm_ultra_v1.yaml
